...
class MicrosoftSecurityReportGenerator:
    ...
    def _categorize_by_product(self, articles):
        """
        Categorize articles by Microsoft Security Product using source field mapping.
        ...
        """
        # Define primary products in the order they should appear
        primary_products = [
            'Entra ID',
            'Microsoft Intune',
            'Microsoft Purview',
        ...
        # These match the exact source names from config_security_products.yaml
        source_mapping = {
            # Entra ID (Identity & Access)
            'Entra ID What\'s New': 'Entra ID',
            "Entra ID What's New": 'Entra ID',
            'Tech Community - Entra Blog': 'Entra ID',
            'Tech Community - Entra (Azure AD) Blog': 'Entra ID',

            # Microsoft Intune (Endpoint Management)
            'Intune What\'s New': 'Microsoft Intune',
            "Intune What's New": 'Microsoft Intune',
            'Tech Community - Intune Blog': 'Microsoft Intune',

            # Microsoft Purview (Data Governance & Compliance)
            'Purview What\'s New': 'Microsoft Purview',
            "Purview What's New": 'Microsoft Purview',
            'Microsoft 365 Compliance What\'s New': 'Microsoft Purview',
            "Microsoft 365 Compliance What's New": 'Microsoft Purview',
            'Tech Community - Purview Blog': 'Microsoft Purview',

            # Office 365 Security
            'Microsoft 365 Admin Center What\'s New': 'Office 365 Security',
            "Microsoft 365 Admin Center What's New": 'Office 365 Security',
            'Office 365 Security What\'s New': 'Office 365 Security',
            "Office 365 Security What's New": 'Office 365 Security',
            'Tech Community - Microsoft 365 Blog': 'Office 365 Security',

            # Microsoft Teams
            'Microsoft Teams What\'s New': 'Microsoft Teams',
            "Microsoft Teams What's New": 'Microsoft Teams',
            'Teams Admin What\'s New': 'Microsoft Teams',
            "Teams Admin What's New": 'Microsoft Teams',
            'Tech Community - Microsoft Teams Blog': 'Microsoft Teams',

            # SharePoint
            'SharePoint What\'s New': 'SharePoint',
            "SharePoint What's New": 'SharePoint',
            'SharePoint Online Updates': 'SharePoint',
            'Tech Community - SharePoint Blog': 'SharePoint',

            # Exchange
            'Exchange Online What\'s New': 'Exchange',
            "Exchange Online What's New": 'Exchange',
            'Exchange Server Updates': 'Exchange',
            'Tech Community - Exchange Blog': 'Exchange',

            # Windows Security
            'Windows Security Updates': 'Windows Security',
            'Windows 11 What\'s New': 'Windows Security',
            "Windows 11 What's New": 'Windows Security',
            'Windows Security Baselines': 'Windows Security',
            'Tech Community - Windows IT Pro Blog': 'Windows Security',

            # Microsoft Copilot (all Copilot products)
            'Security Copilot What\'s New': 'Copilot',
            "Security Copilot What's New": 'Copilot',
            'Microsoft Copilot for Microsoft 365': 'Copilot',
            'Tech Community - Security Copilot Blog': 'Copilot',
            'Tech Community - Microsoft 365 Copilot Blog': 'Copilot',
            'Tech Community - Microsoft Security Blog': 'Copilot',
        }
        ...
        for article in articles:
            ...
            # Check if source contains "Microsoft Security Blog" or "Tech Community"
                ...
                if 'Tech Community' in source:
                    ...
                    if 'entra' in combined or 'azure ad' in combined or 'azure active directory' in combined:
                        products['Entra ID'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (Entra) -> 'Entra ID': {title[:50]}...")
                    elif 'intune' in combined or 'endpoint manager' in combined:
                        products['Microsoft Intune'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (Intune) -> 'Microsoft Intune': {title[:50]}...")
                    elif 'purview' in combined or 'compliance' in combined or 'dlp' in combined:
                        products['Microsoft Purview'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (Purview) -> 'Microsoft Purview': {title[:50]}...")
                    elif ('office 365' in combined or 'm365' in combined) and 'security' in combined:
                        products['Office 365 Security'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (O365) -> 'Office 365 Security': {title[:50]}...")
                    elif 'teams' in combined:
                        products['Microsoft Teams'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (Teams) -> 'Microsoft Teams': {title[:50]}...")
                    elif 'sharepoint' in combined:
                        products['SharePoint'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (SharePoint) -> 'SharePoint': {title[:50]}...")
                    elif 'exchange' in combined:
                        products['Exchange'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (Exchange) -> 'Exchange': {title[:50]}...")
                    elif 'windows' in combined and 'security' in combined:
                        products['Windows Security'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (Windows) -> 'Windows Security': {title[:50]}...")
                    elif 'copilot' in combined:
                        products['Copilot'].append(article)
                        categorized = True
                        logger.debug(f"Mapped Tech Community (Copilot) -> 'Copilot': {title[:50]}...")

                # If Tech Community article wasn't categorized or it's from Security Blog, put in Other
                if not categorized:
                    products['Other Security Updates'].append(article)
                    categorized = True
                    logger.debug(f"Mapped '{source}' -> 'Other Security Updates': {title[:50]}...")
            # Check if source contains "Microsoft Security Blog"
            elif 'Microsoft Security Blog' in source or 'security blog' in source.lower():
                products['Other Security Updates'].append(article)
                categorized = True
                logger.debug(f"Mapped Security Blog -> 'Other Security Updates': {title[:50]}...")
            else:
                # Fallback: try to match source name with product keywords
                source_lower = source.lower()

                if 'entra' in source_lower or 'azure ad' in source_lower:
                    products['Entra ID'].append(article)
                    categorized = True
                elif 'intune' in source_lower:
                    products['Microsoft Intune'].append(article)
                    categorized = True
                elif 'purview' in source_lower:
                    products['Microsoft Purview'].append(article)
                    categorized = True
                elif 'office 365' in source_lower or 'm365' in source_lower:
                    products['Office 365 Security'].append(article)
                    categorized = True
                elif 'teams' in source_lower:
                    products['Microsoft Teams'].append(article)
                    categorized = True
                elif 'sharepoint' in source_lower:
                    products['SharePoint'].append(article)
                    categorized = True
                elif 'exchange' in source_lower:
                    products['Exchange'].append(article)
                    categorized = True
            ...
            if not categorized:
                products['Other Security Updates'].append(article)
                logger.debug(f"Uncategorized source '{source}' -> 'Other Security Updates': {title[:50]}...")
    ...
    def _generate_report_with_tier(self, executive_summary, articles, output_dir, time_period, tier, tier_name, summarizer=None):
        ...
        try:
            ...
            template_data = {
                'title': f"Microsoft Security Product Update Report - {tier_name}",
    ...
    def generate_security_product_report(self, executive_summary, articles, output_dir, time_period="last 30 days"):
        ...
        try:
            ...
            # Prepare template data
            template_data = {
                'title': f"Microsoft Security Product Update Report - {datetime.now().strftime('%B %d, %Y')}",
                'date': datetime.now().strftime('%B %d, %Y at %I:%M %p'),
                'time_period': time_period,
                'total_updates': len(articles),
                'stats': stats,
    ...
    def generate_markdown_report(self, executive_summary, articles, output_dir, time_period="last 30 days"):
        ...
        try:
            ...
            md_content = f"# Microsoft Security Product Update Report\n\n"
